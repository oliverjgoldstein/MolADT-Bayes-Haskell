...
-- | Lazy lightweight probabilistic programming primitives and inference
-- algorithms.  The implementation emphasises laziness so that probabilistic
-- programs can be executed against infinite random trees.
...
-- | Helper that builds a cumulative distribution function from weighted
-- samples.  Each element is duplicated to simplify acceptance sampling in
-- 'lwis'.
...
-- | Mutate each branch of a tree using independent random streams.  The
-- structure of the tree is preserved; only the labels change.
...
mhirreducible p q (Meas m) = do
    ...
    but we use a state monad ('get'/'put') to deal with the other randomness '(g,g1,g2)' -}
    where
          -- | Single Metropolis-Hastings sweep for the irreducible kernel.
          -- Resamples either all random sites or a subset depending on @q@.
          step :: RandomGen g => (Tree,a,Product (Log Double)) -> State g (Tree,a,Product (Log Double))
          step (t,x,w) = do
            -- Randomly change some sites
            g <- get
            let (g1,g2) = split g
...
-- | Enumerate all realised random-choice sites in the partial tree.
getSites :: PTree -> [Site]
getSites p = getSites' p []

-- | Helper that carries the path (as a list of child indices) to each site.
...
-- | Replace the label stored at a particular site of the tree.
...
-- | Apply a set of site updates (typically sampled during MH1 proposals).
mutateNodes :: Tree -> Subst -> Tree
mutateNodes = M.foldrWithKey (\k d t -> mutateNode t k d)

-- | Sample uniformly from a finite list alongside the updated generator.
...
-- | Single-site Metropolis-Hastings following the Lightweight MH approach.
-- Each step resamples one random choice from the execution trace and keeps
-- track of the realised tree using runtime inspection.
...
-- | Inspect the runtime representation of 'Tree' values and build a finite
-- prefix that records which nodes have already been evaluated.
...
-- | Helper for 'helperT' that decodes lazy lists of subtrees.
...
-- | Build a partial tree by observing the currently evaluated part of a
-- 'Tree'.  This is essential for MH1 where only materialised random choices
-- are resampled.
trunc :: Tree -> IO PTree
trunc t = helperT $ asBox t

{-- | Useful function which thins out a list. --}
-- | Return every @n@-th element of an infinite list, preserving laziness.
every :: Int -> [a] -> [a]
every n xs = case drop (n -1) xs of
  (y : ys) -> y : every n ys
  [] -> []

-- | Monadic variant of 'iterate' that records all intermediate states.
...
-- Useful for observing the convergence of long-running inference runs.
