...
The Haskell repo is the aligned baseline. It does not own the large benchmark run or the Python timing bundle, but it does own a local stdout parser-timing command. It reads the Python-exported matrices and runs LWIS or MH over the same standardized `X/y` format.
...
Given a dataset prefix such as `freesolv_moladt_featurized` or `qm9_moladt`, it loads:
...
The aligned Haskell baseline currently uses the Python `freesolv_moladt_featurized` export for FreeSolv and `qm9_moladt` for QM9.
...
This is a stdout-oriented baseline. The Markdown, CSV, graphs, and timing files are written by the Python repo instead, while the Haskell parser-timing command stays local and stdout-only.

## Alignment

The Haskell model is aligned to the Python exported `X/y` format:
