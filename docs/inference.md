...
For the supported FreeSolv dataset prefix `freesolv_moladt_featurized`, it loads:
...
The aligned Haskell consumer currently uses the Python `freesolv_moladt_featurized` export for FreeSolv.
...
The optional final integer argument truncates each split after loading. If you omit it, the Haskell consumer uses the full exported train, validation, and test splits.
