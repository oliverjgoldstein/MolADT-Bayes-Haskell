...
- Added direct CLI support for aligned benchmark inference over Python-exported datasets such as `freesolv_smiles` and `qm9_sdf`.
...
- Added benchmark-alignment tests that verify the Haskell side can read the Python-exported `freesolv_smiles` and `qm9_sdf` datasets.
