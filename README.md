...
This repo does not ship precomputed benchmark results. The reviewer-facing benchmark figures are generated by real non-dry runs from the sibling Python repo, and `results/` is meant to be populated only by those local runs.
...
Reviewer-facing outputs still come from the Python repo:
